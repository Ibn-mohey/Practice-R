...
for(i in 1:40){
  ...
  if(coinFlip){
    dataMatrix[i,] <- dataMatrix[i,] + rep(c(0,3),each=5) #mean of 0 and 3 in the rep 
  }
}
```
...
some of the rows has mean of 3 on  right side 

some of the rows has mean of 0 on  left  side 
---
...
no pattern on the rows

---

## Patterns in rows and columns (margin means)
...
plot(colMeans(dataMatrixOrdered),xlab="Column",ylab="Column Mean",pch=19) #mean of each of rows
```

from the middle plot we can see it tent to shift in the mean as you go across the rows
and there is a shift across the columns also the mean of 0 and the mean of 2 
---
...
it's the best matrix with fewer number of variables that explain the matrix 
...
The principal components are equal to the right singular values if you first scale (subtract the mean, divide by the standard deviation) the variables. then SVD on them u get the V matrix
...
svd picked up the pattern right away into left and rifht single vectors U
---

## Components of the SVD - Variance explained
from the single values of the D matrix
```{r dependson ="oChunk",fig.height=5,fig.width=10}
par(mfrow=c(1,2))
plot(svd1$d,xlab="Column",ylab="Singular value",pch=19)
plot(svd1$d^2/sum(svd1$d^2),xlab="Column",ylab="Prop. of variance explained",pch=19)
```
each value of the D reprset the total variation in the data 

the left has no meaning Raw single values D 
but the proportion of the variance explained >>> first singular is shift and the mean between rows and columns = 40 % variation of data 
the remaining capture with ithers
---
